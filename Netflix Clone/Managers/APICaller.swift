...
struct Constants {
    static let API_KEY = "9c8ff8ccdad813478ea2ae9e6d99d220"
    static let baseURL = "https://api.themoviedb.org"
    static let YoutubeAPI_KEY = "AIzaSyBUttjzlswJfAaUzjsQjsYIjerZElrmxHA"
...
class APICaller {
    ...
    func getTrendingMovies(completion: @escaping (Result<[Title], Error>) -> Void) {
        ...
        guard let url = URL(string:"\(Constants.baseURL)/3/trending/movie/day?api_key=\(Constants.API_KEY)&language=tr-TR&page=1") else {return}
    ...
    func getTrendingTVs(completion: @escaping (Result<[Title], Error>) -> Void) {
        guard let url = URL(string:"\(Constants.baseURL)/3/trending/tv/day?api_key=\(Constants.API_KEY)&language=tr-TR&page=1") else {return}
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)){data, _, error in
            guard let data = data, error == nil else {
                return
            }
    ...
    func search(with query:String, completion: @escaping (Result<[Title], Error>) -> Void){
        ...
        guard let url = URL(string: "\(Constants.baseURL)/3/search/movie?api_key=\(Constants.API_KEY)&query=\(query)&language=tr-TR&page=1") else {
            return
        }
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)) {data, _, error in
            guard let data = data, error == nil else {
    ...
    func getMovie(with query: String, completion: @escaping (Result<VideoElement, Error>) -> Void){
        ...
        guard let url = URL(string: "\(Constants.YoutubeBaseURL)q=\(query)&key=\(Constants.YoutubeAPI_KEY)&language=tr-TR&page=1") else {return}
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)) {data, _, error in
            guard let data = data, error == nil else {
                return
            }
